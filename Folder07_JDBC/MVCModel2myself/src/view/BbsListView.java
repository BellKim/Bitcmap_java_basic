...
//	=================================
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(button == logoutBtn) {
//			JOptionPane.showMessageDialog(null, "로그아웃 버튼");
			Singleton s = Singleton.getInstance();
			this.dispose();
			s.setLoginId(null);
			new loginView();
			
		}else if(button == writeBtn) {
			JOptionPane.showMessageDialog(null, "글쓰기버튼");
...
//		테이블 로우 입력시 동작
		...
		System.out.println("bbbsListView _ actionPerformed내 동작"
				+ "\n선택한 글 정보 : " + list.get(rowNum).toString());
		
		//화면이동 bbslistvie -> bbbsListView
		new bbsDetail();
		si.setBbsbdto(new BbsDto(list.get(rowNum).getSeq(),	//seq
								list.get(rowNum).getId(),	//id
								list.get(rowNum).getTitle(),//title
								list.get(rowNum).getContent(),
								list.get(rowNum).getWdate(),
								list.get(rowNum).getDel(),
								list.get(rowNum).getReadcount()));
//		for (BbsDto bbsDto : columnNames) {
//			
//		}
//		
	}
